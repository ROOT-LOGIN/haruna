...
namespace haruna
{
    ...
    [ContentType("text")]
    [FileExtension(".asm")]
    public sealed class NasmMacroOutlineTaggerProvider : ITaggerProvider
    {
        #region ITaggerProvider Members

        public ITagger<T> CreateTagger<T>(ITextBuffer buffer) where T : ITag
        {
            return new NasmMacroTagger(buffer) as ITagger<T>;

            return buffer.Properties.GetOrCreateSingletonProperty<ITagger<T>>(
                ( ) => new NasmMacroTagger(buffer) as ITagger<T>);
        }
    ...
    public sealed class NasmMacroTagger : ITagger<IOutliningRegionTag>
    {
        const string startHide = "%macro";
        const string endHide = "%endmacro";

        ITextBuffer buffer;
        ITextSnapshot snapshot;
        List<Region> regions;

        public NasmMacroTagger(ITextBuffer buffer)
        {
            this.buffer = buffer;
