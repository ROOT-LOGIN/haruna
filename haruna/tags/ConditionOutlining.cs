...
namespace haruna
{
    ...
    public sealed class NasmConditionOutlineTaggerProvider : ITaggerProvider
    {
        #region ITaggerProvider Members

        public ITagger<T> CreateTagger<T>(ITextBuffer buffer) where T : ITag
        {
            return new NasmConditionTagger(buffer) as ITagger<T>;

            return buffer.Properties.GetOrCreateSingletonProperty<ITagger<T>>(
                ( ) => new NasmConditionTagger(buffer) as ITagger<T>);
        }
    ...
    public sealed class NasmConditionTagger : ITagger<IOutliningRegionTag>
    {
        ...
        public NasmConditionTagger(ITextBuffer buffer)
        {
            this.buffer = buffer;
        ...
        void ReParse( )
        {
            ...
            foreach(var line in newSnapshot.Lines)
            {
                ...
                // %endif, close current region
                    ...
                    && regionStart == signStart)
                {
                    if(m_regionStack.Count == 0) continue;
